import SecurityPosturePanel from "../../components/auth/SecurityPosturePanel";
import { ArrowLeft, Activity, FileCheck, BarChart3, ShieldAlert } from "lucide-react";
import "./auth.css";

export default function SecurityPosturePage({ onNavigate }) {
  return (
      ...
      <nav className="max-w-7xl mx-auto flex items-center justify-between pb-6 mb-8 border-b border-slate-800">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg text-white">MedTrack Security Posture & Cyber Risk</span>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
            <Activity size={14} /> CIS Benchmark v8 Engine Active
          </span>
        </div>
      </nav>
        ...
        <section className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-800 via-emerald-955 to-slate-900 p-8 border border-slate-700/50 shadow-2xl">
          <div className="relative z-10 max-w-3xl space-y-4">
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight">
              Security Posture & Cyber Risk Assessment Subsystem
            </h1>

            <p className="text-slate-300 text-sm md:text-base leading-relaxed">
              Evaluate real-time application security posture against CIS Benchmark v8, NIST SP 800-53, and ISO 27001 domain standards, monitor control health, and generate posture scorecards.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
              <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/60 border border-slate-700/40">
                ...
                <div>
                  <div className="text-xs text-slate-400">CIS Controls</div>
                  <div className="text-xs font-semibold text-white">Automated Checks</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/60 border border-slate-700/40">
                ...
                <div>
                  ...
                  <div className="text-xs font-semibold text-white">0-100 Scorecard</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/60 border border-slate-700/40">
                ...
                <div>
                  <div className="text-xs text-slate-400">Breach Alerts</div>
                  <div className="text-xs font-semibold text-white">Threshold Enforcement</div>
                </div>
              </div>
            </div>
          </div>
