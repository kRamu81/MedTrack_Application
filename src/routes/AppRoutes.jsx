...
// Page Imports (Ensure these paths match your actual folder structure)
...
export default function AppRouter({ currentPage, onNavigate }) {
  const { user } = useAuth();

  // Helper function to protect routes
  // If user exists, show the component; otherwise, redirect to Login
  const ProtectedRoute = (Component) => {
    return user ? <Component onNavigate={onNavigate} /> : <LoginPage onNavigate={onNavigate} />;
  };
  ...
  switch (currentPage) {
    ...
    case "updatetask": 
      return ProtectedRoute(UpdateTask);
